...
import { Loader2, GraduationCap, LogOut, BookOpenText, Sparkles } from 'lucide-react';
...
export default function DashboardSiswaPage() {
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center"><Loader2 className="animate-spin text-[#8DAA7B] w-8 h-8" /></div>;
return (
      ...
      <div className="max-w-5xl mx-auto space-y-8 sm:space-y-12">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            <h1 className="text-2xl sm:text-3xl font-black text-[#2D332D]">Halo, {studentName}! 👋</h1>
          ...
          {classes.length > 0 ? (
            classes.map((item, index) => (
              ...
              >
                ...
                <div className="min-w-0"> {/* Mencegah teks meluap */}
            ...
            <div className="col-span-full text-center py-16 sm:py-20 bg-white rounded-[2rem] border-2 border-dashed border-[#DDE2D8] px-4">
              <p className="text-[#6B705C] font-bold text-sm sm:text-base">Belum ada kelas yang terdaftar, tetap semangat ya! 🌟</p>
            </div>
          )}
        </div>
      </div>
