...
export default function LoginOrangTuaPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (!name.trim() || !password.trim()) {
      alert("Mohon isi nama dan kata sandi ya!");
      return;
    }
    ...
    <main className="min-h-screen bg-[#F5F7F2] flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-[#6B705C] text-xs font-bold mt-1">Silakan masuk dengan akun terdaftar</p>
        </div>

        <form onSubmit={handleLogin} className="space-y-4">
          {/* Input Nama */}
          <div className="space-y-1">
            <label className="text-xs font-extrabold text-[#4A5043] ml-1">Nama Orang Tua:</label>
